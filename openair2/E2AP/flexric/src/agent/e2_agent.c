...
static
char const* e2ap_msg_type_to_str_agent(e2_msg_type_t type)
{
  switch(type) {
    case NONE_E2_MSG_TYPE: return "NONE_E2_MSG_TYPE";
    case RIC_SUBSCRIPTION_REQUEST: return "RIC_SUBSCRIPTION_REQUEST";
    case RIC_SUBSCRIPTION_RESPONSE: return "RIC_SUBSCRIPTION_RESPONSE";
    case RIC_SUBSCRIPTION_FAILURE: return "RIC_SUBSCRIPTION_FAILURE";
    case RIC_SUBSCRIPTION_DELETE_REQUEST: return "RIC_SUBSCRIPTION_DELETE_REQUEST";
    case RIC_SUBSCRIPTION_DELETE_RESPONSE: return "RIC_SUBSCRIPTION_DELETE_RESPONSE";
    case RIC_SUBSCRIPTION_DELETE_FAILURE: return "RIC_SUBSCRIPTION_DELETE_FAILURE";
    case RIC_INDICATION: return "RIC_INDICATION";
    case RIC_CONTROL_REQUEST: return "RIC_CONTROL_REQUEST";
    case RIC_CONTROL_ACKNOWLEDGE: return "RIC_CONTROL_ACKNOWLEDGE";
    case RIC_CONTROL_FAILURE: return "RIC_CONTROL_FAILURE";
    case E2_SETUP_REQUEST: return "E2_SETUP_REQUEST";
    case E2_SETUP_RESPONSE: return "E2_SETUP_RESPONSE";
    case E2_SETUP_FAILURE: return "E2_SETUP_FAILURE";
    case E2AP_RESET_REQUEST: return "E2AP_RESET_REQUEST";
    case E2AP_RESET_RESPONSE: return "E2AP_RESET_RESPONSE";
    case RIC_SERVICE_UPDATE: return "RIC_SERVICE_UPDATE";
    case E2_NODE_CONFIGURATION_UPDATE: return "E2_NODE_CONFIGURATION_UPDATE";
    case E2_CONNECTION_UPDATE: return "E2_CONNECTION_UPDATE";
    case E2_CONNECTION_UPDATE_ACKNOWLEDGE: return "E2_CONNECTION_UPDATE_ACKNOWLEDGE";
    case E2_CONNECTION_UPDATE_FAILURE: return "E2_CONNECTION_UPDATE_FAILURE";
    case E2AP_ERROR_INDICATION: return "E2AP_ERROR_INDICATION";
    default: return "UNKNOWN_E2_MSG_TYPE";
  }
}

static
char const* sctp_msg_type_to_str_agent(sctp_msg_type_t type)
{
  switch(type) {
    case SCTP_MSG_PAYLOAD: return "SCTP_MSG_PAYLOAD";
    case SCTP_MSG_NOTIFICATION: return "SCTP_MSG_NOTIFICATION";
    default: return "UNKNOWN_SCTP_MSG_TYPE";
  }
}

static
void log_byte_array_info_ag(char const* prefix, char const* name, byte_array_t const* ba)
{
  assert(prefix != NULL);
  assert(name != NULL);
  assert(ba != NULL);
  ...
  size_t const dump = ba->len < 32 ? ba->len : 32;
  printf("%s %s.first_%zu_bytes =", prefix, name, dump);
...
async_event_t next_async_event_agent(e2_agent_t* ag)
{
  ...
  if(fd == -1){ // no event happened. Just for checking the stop_token condition
    ...
    printf("[E2-AGENT][RX] next_async_event_agent: fd=%d, sctp_msg.type=%s(%d)\n",
           fd,
           sctp_msg_type_to_str_agent(e.msg.type),
           e.msg.type);

    if (e.msg.ba.buf != NULL) {
      log_byte_array_info_ag("[E2-AGENT][RX]", "raw_sctp_payload", &e.msg.ba);
    }
  ...
  while(ag->stop_token == false){
    ...
    switch(e.type){
      case SCTP_MSG_ARRIVED_EVENT:
        {
          printf("[E2-AGENT][RX] ================================================\n");
          printf("[E2-AGENT][RX] SCTP_MSG_ARRIVED_EVENT triggered\n");
          ...
          printf("[E2-AGENT][RX] e2ap_msg_dec_ag() done\n");
          printf("[E2-AGENT][RX] decoded msg.type = %s(%d)\n",
                 e2ap_msg_type_to_str_agent(msg.type), msg.type);

          if (msg.type == RIC_CONTROL_REQUEST) {
            printf("[E2-AGENT][CTRL] >>> RIC_CONTROL_REQUEST received at E2 agent <<<\n");
          }

          e2ap_msg_t ans = e2ap_msg_handle_agent(ag, &msg);
          defer({ e2ap_msg_free_ag(&ag->ap, &ans); });

          printf("[E2-AGENT][RX] e2ap_msg_handle_agent() done\n");
          printf("[E2-AGENT][RX] answer msg.type = %s(%d)\n",
                 e2ap_msg_type_to_str_agent(ans.type), ans.type);

          if (ans.type == RIC_CONTROL_ACKNOWLEDGE) {
            printf("[E2-AGENT][CTRL] handler generated RIC_CONTROL_ACKNOWLEDGE\n");
          } else if (ans.type == RIC_CONTROL_FAILURE) {
            printf("[E2-AGENT][CTRL] handler generated RIC_CONTROL_FAILURE\n");
          }
          ...
          if(ans.type != NONE_E2_MSG_TYPE){
            ...
            printf("[E2-AGENT][TX] e2ap_msg_enc_ag() done for ans.type=%s(%d)\n",
                   e2ap_msg_type_to_str_agent(ans.type), ans.type);
            log_byte_array_info_ag("[E2-AGENT][TX]", "encoded_answer", &ba_ans);

            printf("[E2-AGENT][TX] calling e2ap_send_bytes_agent()\n");
            e2ap_send_bytes_agent(&ag->ep, ba_ans);
            printf("[E2-AGENT][TX] e2ap_send_bytes_agent() returned\n");
          }
...
void e2_start_agent(e2_agent_t* ag)
{
  ...
  printf("[E2-AGENT][INIT] E2 SETUP-REQUEST tx\n");
  byte_array_t ba = e2ap_enc_setup_request_ag(&ag->ap, &sr); 
  defer({free_byte_array(ba); } ); 
  ...
  printf("[E2-AGENT][INIT] setup request pending timer fd = %d, wait_ms = %ld\n", fd_timer, wait_ms);
...
void e2_send_control_acknowledge(e2_agent_t* ag, const ric_control_acknowledge_t* ca)
{
  ...
  printf("[E2-AGENT][CTRL][TX] e2_send_control_acknowledge() called\n");

  byte_array_t ba = e2ap_enc_control_acknowledge_ag(&ag->ap, ca);
  log_byte_array_info_ag("[E2-AGENT][CTRL][TX]", "control_ack", &ba);

  printf("[E2-AGENT][CTRL][TX] sending CONTROL_ACKNOWLEDGE\n");
  e2ap_send_bytes_agent(&ag->ep, ba);
  printf("[E2-AGENT][CTRL][TX] CONTROL_ACKNOWLEDGE sent\n");
...
void e2_send_control_failure(e2_agent_t* ag, const ric_control_failure_t* cf)
{
  ...
  printf("[E2-AGENT][CTRL][TX] e2_send_control_failure() called\n");

  byte_array_t ba = e2ap_enc_control_failure_ag(&ag->ap, cf);
  log_byte_array_info_ag("[E2-AGENT][CTRL][TX]", "control_failure", &ba);

  printf("[E2-AGENT][CTRL][TX] sending CONTROL_FAILURE\n");
  e2ap_send_bytes_agent(&ag->ep, ba);
  printf("[E2-AGENT][CTRL][TX] CONTROL_FAILURE sent\n");
