...
static
void log_byte_array_info_mha(char const* prefix, char const* name, uint8_t const* buf, size_t len)
{
  assert(prefix != NULL);
  assert(name != NULL);
  ...
  size_t const dump = len < 32 ? len : 32;
...
static
void log_ric_gen_id_mha(char const* prefix, ric_gen_id_t const* id)
{
  assert(prefix != NULL);
  assert(id != NULL);
...
e2ap_msg_t e2ap_msg_handle_agent(e2_agent_t* ag, const e2ap_msg_t* msg)
{
  ...
  printf("[E2-AGENT][HANDLER] e2ap_msg_handle_agent: dispatch msg_type=%d\n", msg_type);
...
e2ap_msg_t e2ap_handle_control_request_agent(e2_agent_t* ag, const e2ap_msg_t* msg)
{
  ...
  printf("[E2-AGENT][CTRL] ==================================================\n");
  printf("[E2-AGENT][CTRL] e2ap_handle_control_request_agent: start\n");
  ...
  printf("[E2-AGENT][CTRL] received RIC_CONTROL_REQUEST\n");
  log_ric_gen_id_mha("[E2-AGENT][CTRL]", &ctrl_req->ric_id);
  printf("[E2-AGENT][CTRL] ack_req = %d\n", *ctrl_req->ack_req);

  log_byte_array_info_mha("[E2-AGENT][CTRL]", "ctrl_hdr", ctrl_req->hdr.buf, ctrl_req->hdr.len);
  log_byte_array_info_mha("[E2-AGENT][CTRL]", "ctrl_msg", ctrl_req->msg.buf, ctrl_req->msg.len);

  sm_ctrl_req_data_t data = {
    .ctrl_hdr = ctrl_req->hdr.buf,
  ...
  printf("[E2-AGENT][CTRL] lookup sm_plugin_ag() with ran_func_id = %u\n", ran_func_id);

  sm_agent_t* sm = sm_plugin_ag(&ag->plugin, ran_func_id);
  assert(sm != NULL && "sm_plugin_ag returned NULL");

  printf("[E2-AGENT][CTRL] sm_plugin_ag() returned valid SM\n");
  printf("[E2-AGENT][CTRL] calling sm->proc.on_control()\n");

  sm_ctrl_out_data_t ctrl_ans = sm->proc.on_control(sm, &data);
  defer({ free_sm_ctrl_out_data(&ctrl_ans); });

  printf("[E2-AGENT][CTRL] sm->proc.on_control() returned\n");
  printf("[E2-AGENT][CTRL] ctrl_out.len_out = %zu\n", ctrl_ans.len_out);

  byte_array_t* ba_ctrl_ans = ba_from_ctrl_out(&ctrl_ans);

  if (ba_ctrl_ans != NULL) {
    log_byte_array_info_mha("[E2-AGENT][CTRL]", "ctrl_outcome", ba_ctrl_ans->buf, ba_ctrl_ans->len);
  } else {
    printf("[E2-AGENT][CTRL] ctrl_outcome = NULL\n");
  }

  ric_control_acknowledge_t ric_ctrl_ack = {
    .ric_id = ctrl_req->ric_id,
  ...
  printf("[E2-AGENT][CTRL] preparing RIC_CONTROL_ACKNOWLEDGE\n");
  log_ric_gen_id_mha("[E2-AGENT][CTRL]", &ric_ctrl_ack.ric_id);
  ...
  printf("[E2-AGENT][CTRL] e2ap_handle_control_request_agent: end\n");
