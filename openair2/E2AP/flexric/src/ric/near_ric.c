...
static
void log_byte_array_info_nr(char const* prefix, char const* name, byte_array_t const* ba)
{
  assert(prefix != NULL);
  assert(name != NULL);
  assert(ba != NULL);
...
static
void log_ric_gen_id_nr(char const* prefix, ric_gen_id_t const* id)
{
  assert(prefix != NULL);
  assert(id != NULL);

  printf("%s ric_req_id = %u\n", prefix, id->ric_req_id);
  printf("%s ric_inst_id = %u\n", prefix, id->ric_inst_id);
  printf("%s ran_func_id = %u\n", prefix, id->ran_func_id);
}

static
void log_pending_event_nr(char const* prefix, pending_event_ric_t const* ev, int fd_timer, long wait_ms)
{
  assert(prefix != NULL);
  assert(ev != NULL);

  printf("%s pending_event.ev = %d\n", prefix, ev->ev);
  printf("%s pending_event.timer_fd = %d\n", prefix, fd_timer);
  printf("%s pending_event.wait_ms = %ld\n", prefix, wait_ms);
  log_ric_gen_id_nr(prefix, &ev->id);
}
...
void sctp_msg_arrived_event(void* arg)
{
  ...
  printf("[RIC-CORE][RX] sctp_msg_arrived_event: decoded msg.type = %d\n", msg.type);
  ...
  printf("[RIC-CORE][RX] e2ap_msg_handle_ric() produced ans.type = %d\n", ans.type);
...
ric_control_request_t generate_control_request(near_ric_t* ric, sm_ric_t* sm, void* ctrl)
{
  ...
  const ric_gen_id_t ric_id = {
    .ric_req_id = ric->req_id++,
    .ric_inst_id = 0,
    .ran_func_id = sm->ran_func_id
  };

  printf("[RIC-CORE][CTRL] generate_control_request: start\n");
  log_ric_gen_id_nr("[RIC-CORE][CTRL]", &ric_id);
  ...
  *ctrl_req.ack_req = RIC_CONTROL_REQUEST_ACK;
  ...
  printf("[RIC-CORE][CTRL] ack_req = %d\n", *ctrl_req.ack_req);

  sm_ctrl_req_data_t data = sm->proc.on_control_req(sm, ctrl);

  printf("[RIC-CORE][CTRL] on_control_req() returned\n");
  printf("[RIC-CORE][CTRL] data.len_hdr = %zu\n", data.len_hdr);
  printf("[RIC-CORE][CTRL] data.len_msg = %zu\n", data.len_msg);
  ...
  log_byte_array_info_nr("[RIC-CORE][CTRL]", "ctrl_hdr", &ctrl_req.hdr);
  log_byte_array_info_nr("[RIC-CORE][CTRL]", "ctrl_msg", &ctrl_req.msg);

  printf("[RIC-CORE][CTRL] generate_control_request: end\n");
...
void control_service_near_ric(near_ric_t* ric, global_e2_node_id_t const* id, uint16_t ran_func_id, void* ctrl)
{
  ...
  printf("[RIC-CORE][CTRL] ==================================================\n");
  printf("[RIC-CORE][CTRL] control_service_near_ric: start\n");
  printf("[RIC-CORE][CTRL] requested ran_func_id = %u\n", ran_func_id);

  sm_ric_t* sm = sm_plugin_ric(&ric->plugin, ran_func_id);
  assert(sm != NULL && "sm_plugin_ric returned NULL");

  printf("[RIC-CORE][CTRL] sm->ran_func_id = %u\n", sm->ran_func_id);

  ric_control_request_t ctrl_req = generate_control_request(ric, sm, ctrl);

  printf("[RIC-CORE][CTRL] generated control request\n");
  log_ric_gen_id_nr("[RIC-CORE][CTRL]", &ctrl_req.ric_id);

  // A pending event is created along with a timer of 3000 ms,
  // after which an event will be generated
  pending_event_ric_t ev = {
  ...
  log_pending_event_nr("[RIC-CORE][CTRL]", &ev, fd_timer, wait_ms);

  {
    lock_guard(&ric->pend_mtx);
    bi_map_insert(&ric->pending, &fd_timer, sizeof(fd_timer), &ev, sizeof(ev));
    printf("[RIC-CORE][CTRL] pending control event inserted into bi_map\n");
  }

  byte_array_t ba_msg = e2ap_enc_control_request_ric(&ric->ap, &ctrl_req);

  printf("[RIC-CORE][CTRL] e2ap_enc_control_request_ric() returned\n");
  log_byte_array_info_nr("[RIC-CORE][CTRL]", "encoded_e2ap_ctrl_req", &ba_msg);

  printf("[RIC-CORE][CTRL] calling e2ap_send_bytes_ric()\n");
  e2ap_send_bytes_ric(&ric->ep, id, ba_msg);
  printf("[RIC-CORE][CTRL] e2ap_send_bytes_ric() returned\n");

  printf("[RIC-CORE][CTRL] CONTROL SERVICE sent\n");
  printf("[RIC-CORE][CTRL] control_service_near_ric: end\n");
...
uint16_t fwd_ric_control_request(near_ric_t* ric, global_e2_node_id_t const* id, ric_control_request_t const* cr, void (*f)(e2ap_msg_t const* msg))
{
  ...
  printf("[RIC-CORE][CTRL-FWD] fwd_ric_control_request: start\n");

  uint16_t const ric_req_id = ric->req_id++;
  *(uint16_t*)&cr->ric_id.ric_req_id = ric_req_id;

  log_ric_gen_id_nr("[RIC-CORE][CTRL-FWD]", &cr->ric_id);
  ...
  log_pending_event_nr("[RIC-CORE][CTRL-FWD]", &ev, fd_timer, wait_ms);

  {
    lock_guard(&ric->pend_mtx);
    bi_map_insert(&ric->pending, &fd_timer, sizeof(fd_timer), &ev, sizeof(ev));
    printf("[RIC-CORE][CTRL-FWD] pending control event inserted\n");
  }

  byte_array_t ba_msg = e2ap_enc_control_request_ric(&ric->ap, cr);
  defer({ free_byte_array(ba_msg); });

  log_byte_array_info_nr("[RIC-CORE][CTRL-FWD]", "encoded_e2ap_ctrl_req", &ba_msg);

  printf("[RIC-CORE][CTRL-FWD] calling e2ap_send_bytes_ric()\n");
  e2ap_send_bytes_ric(&ric->ep, id, ba_msg);
  printf("[RIC-CORE][CTRL-FWD] e2ap_send_bytes_ric() returned\n");

  printf("[RIC-CORE][CTRL-FWD] CONTROL SERVICE sent\n");
  printf("[RIC-CORE][CTRL-FWD] fwd_ric_control_request: end\n");
