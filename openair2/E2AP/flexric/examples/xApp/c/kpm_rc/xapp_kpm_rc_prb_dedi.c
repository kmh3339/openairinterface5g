...
typedef struct {
  ...
  uint16_t last_sent_dedicated_prb;
                                ...
                                uint64_t cycle)
{
  ...
  for (int i = 0; i < NUM_SLICE_PROFILES; i++) {
    ...
    uint16_t const previous = st->has_ctrl ?
        st->last_sent_dedicated_prb : st->profile->dedicated_prb;
    ...
    if (stale) {
      raw_demand = st->has_ctrl ?
          (double)st->last_sent_dedicated_prb / (double)TOTAL_PRB_BUDGET_RATIO : 1.0;
  ...
  for (size_t u = 0; u < MAX_OBSERVED_UE && n < max_candidates; u++) {
    ...
    if (VERIFY_FORCE_PRB_MAX_RATIO > 0) {
      printf(LOG_PREFIX " verify_force_prb_max: cycle=%" PRIu64
             " slice=%s calculated_target=%u forced_target=%u force_ratio=%u\n",
             cycle,
             st->profile->profile_name,
    ...
    if (msg_min > desired) {
      printf(LOG_PREFIX " warning: adjusted control msg min for slice=%s"
             " min=%u target=%u to keep min<=dedicated<=max\n",
             st->profile->profile_name,
             (unsigned)msg_min,
             (unsigned)desired);
      msg_min = desired;
    ...
    printf(LOG_PREFIX " decision: cycle=%" PRIu64
           " slice=%s current=%u target=%u delta=%u hysteresis_ok=%d"
    ...
    if (!st->has_ctrl) {
      printf(LOG_PREFIX " initial allocation: cycle=%" PRIu64
             " slice=%s target=%u current_default=%u\n",
             cycle,
             st->profile->profile_name,
             (unsigned)desired,
             (unsigned)previous);
    ...
    printf(LOG_PREFIX " allocation_before: cycle=%" PRIu64
           " slice=%s ue_key=%" PRIu64 " current=%u target=%u diff=%u"
    ...
    if (!should_send) {
      printf(LOG_PREFIX " skip control send: cycle=%" PRIu64
             ...
             " reason=%s target=%u current=%u delta=%u"
      ...
      printf(LOG_PREFIX " allocation_after: cycle=%" PRIu64
             " slice=%s current=%u target=%u sent=0 reason=%s\n",
             cycle,
             st->profile->profile_name,
             (unsigned)previous,
             (unsigned)desired,
     ...
     * policy. Keep dedicated at the baseline target and clamp max to the same
     * target so each closed-loop allocation changes the effective quota.
     */
    c->obs.profile.min_prb = msg_min;
    c->obs.profile.max_prb = desired;
    c->obs.profile.dedicated_prb = desired;
  ...
  for (int i = 0; i < NUM_SLICE_PROFILES; i++) {
    ...
    printf(LOG_PREFIX " skip control send: cycle=%" PRIu64
           " slice=%s reason=no observed UE target=%u current=%u\n",
           cycle,
           st->profile->profile_name,
           (unsigned)target[i],
           (unsigned)previous_prb[i]);
    printf(LOG_PREFIX " allocation_after: cycle=%" PRIu64
           " slice=%s current=%u target=%u sent=0 reason=no observed UE\n",
           cycle,
           st->profile->profile_name,
           (unsigned)previous_prb[i],
           (unsigned)target[i]);
...
void mark_control_sent(control_candidate_t const* c, int64_t now_us)
{
  ...
  g_slice_states[c->slice_idx].last_sent_dedicated_prb = c->desired_prb;
  ...
  while (!atomic_load(&g_control_stop)) {
    ...
    if (args->nodes->len == 0 && n_candidates > 0) {
      ...
      for (size_t c_idx = 0; c_idx < n_candidates; c_idx++) {
        ...
        printf(LOG_PREFIX " allocation_after: cycle=%" PRIu64
               " slice=%s current=%u target=%u sent=0 reason=no E2 node/control handle\n",
               cycle,
               c->obs.profile.profile_name,
    ...
    for (size_t i = 0; i < args->nodes->len; ++i) {
      ...
      if (!rc_supports_slice_prb_quota(n->rf[idx].defn.rc.ctrl)) {
        ...
        for (size_t c_idx = 0; c_idx < n_candidates; c_idx++) {
          ...
          printf(LOG_PREFIX " control_send_result: cycle=%" PRIu64
                 ...
                 c->obs.profile.profile_name);
          printf(LOG_PREFIX " allocation_after: cycle=%" PRIu64
                 " slice=%s current=%u target=%u sent=0 reason=unsupported control message\n",
                 cycle,
                 c->obs.profile.profile_name,
                 (unsigned)c->previous_prb,
                 (unsigned)c->desired_prb);
      ...
      for (size_t c_idx = 0; c_idx < n_candidates; c_idx++) {
        ...
        printf(LOG_PREFIX " allocation_after: cycle=%" PRIu64
               " slice=%s current=%u target=%u sent=%d\n",
               cycle,
               c->obs.profile.profile_name,
        ...
        printf("[RIC-XAPP][CTRL] sent profile=%s dedicated=%u success=%d and freed RC control message\n",
               c->obs.profile.profile_name,
               (unsigned)c->obs.profile.dedicated_prb,
               ans.success ? 1 : 0);
      }
