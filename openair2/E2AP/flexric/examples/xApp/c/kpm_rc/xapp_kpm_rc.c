...
void sm_cb_kpm(sm_ag_if_rd_t const* rd)
{
  ...
  static int counter = 1;
  {
    ...
    for (size_t i = 0; i < msg_frm_3->ue_meas_report_lst_len; i++) {
      ...
      printf("[RIC-XAPP][KPM] saved UE ID for future RC control\n");
      log_saved_ue_id("[RIC-XAPP][KPM]", &ue_id);      
...
seq_ran_param_t fill_drb_id_param(void)
{
  ...
  drb_param.ran_param_val.flag_true->int_ran = 7;

  printf("[RIC-XAPP][CTRL] fill_drb_id_param: ran_param_id=%u, DRB_ID=%ld\n",
         drb_param.ran_param_id,
         drb_param.ran_param_val.flag_true->int_ran);
...
seq_ran_param_t fill_qos_flows_param(void)
{
  ...
  printf("[RIC-XAPP][CTRL] fill_qos_flows_param: ran_param_id=%u\n", qos_param.ran_param_id);
  printf("[RIC-XAPP][CTRL]   QOS_FLOW_ID=%ld\n", rps[0].ran_param_val.flag_true->int_ran);
  printf("[RIC-XAPP][CTRL]   QOS_FLOW_MAPPING_IND=%ld\n", rps[1].ran_param_val.flag_false->int_ran);
                      ...
                      e2sm_rc_ctrl_msg_frmt_1_t* msg)
{
  ...
  for (size_t i = 0; i < sz; i++) {
    assert(cmp_str_ba("QoS flow mapping configuration", ctrl_act[i].name) == 0 &&
           "Add requested CONTROL Action. At the moment, only QoS flow mapping configuration supported");

    printf("[RIC-XAPP][CTRL] fill_rc_ctrl_act: matched action name = QoS flow mapping configuration\n");

    hdr->ctrl_act_id = QoS_flow_mapping_configuration_7_6_2_1;

    printf("[RIC-XAPP][CTRL] fill_rc_ctrl_act: ctrl_act_id = %d\n", hdr->ctrl_act_id);
    ...
    printf("[RIC-XAPP][CTRL] fill_rc_ctrl_act: sz_ran_param = %zu\n", msg->sz_ran_param);
...
rc_ctrl_req_data_t gen_rc_ctrl_msg(ran_func_def_ctrl_t const* ran_func)
{
  ...
  printf("[RIC-XAPP][CTRL] gen_rc_ctrl_msg: start\n");

  for (size_t i = 0; i < ran_func->sz_seq_ctrl_style; i++) {
    assert(cmp_str_ba("Radio Bearer Control", ran_func->seq_ctrl_style[i].name) == 0 &&
           "Add requested CONTROL Style. At the moment, only Radio Bearer Control supported");

    printf("[RIC-XAPP][CTRL] gen_rc_ctrl_msg: control style[%zu] = Radio Bearer Control\n", i);
    ...
    printf("[RIC-XAPP][CTRL] gen_rc_ctrl_msg: hdr.format = %d\n", rc_ctrl.hdr.format);
    printf("[RIC-XAPP][CTRL] gen_rc_ctrl_msg: ric_style_type = %d\n", rc_ctrl.hdr.frmt_1.ric_style_type);
    log_saved_ue_id("[RIC-XAPP][CTRL]", &rc_ctrl.hdr.frmt_1.ue_id);
    ...
    printf("[RIC-XAPP][CTRL] gen_rc_ctrl_msg: msg.format = %d\n", rc_ctrl.msg.format);
  ...
  printf("[RIC-XAPP][CTRL] gen_rc_ctrl_msg: end\n");
  ...
  for (size_t i = 0; i < nodes.len; ++i) {
    ...
    // if CONTROL Service is supported by E2 node, send CONTROL message
        if (n->rf[idx].defn.rc.ctrl != NULL) {
          printf("[RIC-XAPP][CTRL] --------------------------------------------------\n");
          printf("[RIC-XAPP][CTRL] preparing CONTROL for node index = %zu\n", i);
          printf("[RIC-XAPP][CTRL] RC ran function id = %d\n", RC_ran_function);
          ...
          printf("[RIC-XAPP][CTRL] control_sm_xapp_api() returned\n");

          free_rc_ctrl_req_data(&rc_ctrl);
          printf("[RIC-XAPP][CTRL] freed RC control message\n");
    }
  }
  ////////////
  // END RC
