...
static
void log_ran_param_value_rc(char const* prefix, ran_param_val_t const* val)
{
  assert(prefix != NULL);
  assert(val != NULL);
  ...
  switch(val->type) {
    case ELEMENT_KEY_FLAG_TRUE_RAN_PARAMETER_VAL_TYPE:
      printf("%s ran_param_val.type = FLAG_TRUE\n", prefix);
      if (val->flag_true != NULL) {
        printf("%s flag_true->type = %d\n", prefix, val->flag_true->type);
        if (val->flag_true->type == INTEGER_RAN_PARAMETER_VALUE) {
          printf("%s flag_true->int_ran = %ld\n", prefix, val->flag_true->int_ran);
        }
      }
      break;

    case ELEMENT_KEY_FLAG_FALSE_RAN_PARAMETER_VAL_TYPE:
      printf("%s ran_param_val.type = FLAG_FALSE\n", prefix);
      if (val->flag_false != NULL) {
        printf("%s flag_false->type = %d\n", prefix, val->flag_false->type);
        if (val->flag_false->type == INTEGER_RAN_PARAMETER_VALUE) {
          printf("%s flag_false->int_ran = %ld\n", prefix, val->flag_false->int_ran);
        }
      }
      break;

    case LIST_RAN_PARAMETER_VAL_TYPE:
      printf("%s ran_param_val.type = LIST\n", prefix);
      if (val->lst != NULL) {
        printf("%s list size = %zu\n", prefix, val->lst->sz_lst_ran_param);
      }
      break;

    case STRUCTURE_RAN_PARAMETER_VAL_TYPE:
      printf("%s ran_param_val.type = STRUCTURE\n", prefix);
      break;

    default:
      printf("%s ran_param_val.type = UNKNOWN(%d)\n", prefix, val->type);
      break;
  }
}
...
sm_ag_if_ans_t write_ctrl_rc_sm(void const* data)
{
  ...
  for (size_t i = 0; i < ctrl->msg.frmt_1.sz_ran_param; i++) {
    ...
    log_ran_param_value_rc("[RAN-RC][CTRL]", &ran_param[i].ran_param_val);
  }
  ...
  for (size_t list_idx = 0; list_idx < ran_param[1].ran_param_val.lst->sz_lst_ran_param; list_idx++) {
    ...
    printf("[RAN-RC][CTRL] list[%zu].struct_size = %zu\n",
           list_idx, lrp->ran_param_struct.sz_ran_param_struct);
    ...
    assert(lrp->ran_param_struct.ran_param_struct[0].ran_param_val.flag_true->type == INTEGER_RAN_PARAMETER_VALUE);
    ...
    assert(lrp->ran_param_struct.ran_param_struct[1].ran_param_val.flag_false->type == INTEGER_RAN_PARAMETER_VALUE);
