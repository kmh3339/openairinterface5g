    ...
    case OCTET_STRING_TEST_COND_VALUE: {
      if (test_cond_value->octet_string_value->len == 1) {
        ...
        *sd = malloc(sizeof(uint32_t));
        DevAssert(*sd != NULL);
        **sd = ((uint32_t)buf[1] << 16) | ((uint32_t)buf[2] << 8) | (uint32_t)buf[3];
      }

      if (*sd == NULL) {
        printf("[E2 AGENT][E2SM-KPM] Condition NSSAI (sst=%u, sd=ANY).\n", *sst);
      } else {
        printf("[E2 AGENT][E2SM-KPM] Condition NSSAI (sst=%u, sd=%06x).\n", *sst, **sd);
      }
      break;
    }
...
static bool nssai_matches(nssai_t a_nssai, uint8_t b_sst, const uint32_t *b_sd)
{
  ...
  /* xApp sent only SST -> SD wildcard */
  if (b_sd == NULL)
    return true;

  AssertFatal(*b_sd <= 0xffffff, "illegal SD %u\n", *b_sd);
  return a_nssai.sd == *b_sd;
}
