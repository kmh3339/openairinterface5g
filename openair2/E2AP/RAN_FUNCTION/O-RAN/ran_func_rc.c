...
sm_ag_if_ans_t write_ctrl_rc_sm(void const* data)
{
 ...
 printf("[RAN-RC][CTRL] matched action: Slice-level PRB quota\n");
  ...
  printf("[RAN-RC][CTRL] NOTE: slice PRB quota stored for NR MAC DL scheduler enforcement.\n");
  printf("[RAN-RC][CTRL] NOTE: current implementation does not enforce this quota in the UL scheduler.\n");
}
