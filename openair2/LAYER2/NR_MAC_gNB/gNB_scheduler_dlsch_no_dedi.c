...
#define RC_SLICE_CSV_ENABLE 1
#define RC_SLICE_CSV_DEFAULT_PATH "/home/user/oai-runner/logs/latest/dl_sched_prb_log.csv"
...
#define HALFWORD 16
#define WORD 32
#define RC_SLICE_LOG_ENABLE 1
#define RC_SLICE_LOG_PERIOD 500

#define RC_SLICE_SHOULD_LOG(frame, slot) \
  (RC_SLICE_LOG_ENABLE && ((frame) % RC_SLICE_LOG_PERIOD == 0) && ((slot) == 0))
...
static int find_policy_idx_by_sst(gNB_MAC_INST *mac, uint8_t sst);
static int find_ue_sst(const NR_UE_info_t *UE, uint8_t *out_sst);

static void rc_slice_csv_write_alloc(frame_t frame,
                                     slot_t slot,
                                     const char *event,
                                     uint16_t rnti,
                                     uint8_t sst,
                                     ...
                                     int slice_used,
                                     int slice_max,
                                     uint16_t constrained_min_rb,
                                     uint16_t constrained_max_rb);
                                       ...
                                       int current_harq_pid)
{
  ...
  * this helps allocate retransmission when TDA changes (e.g. new nrOfSymbols > old nrOfSymbols) */
  ...
  /* CSV log for DL retransmission PRB allocation.
   * Retransmissions are handled outside the slice-budget accounting path,
   * so slice_used/slice_max are stored as -1 while sst/policy_idx are still recorded.
   */
  uint8_t ue_sst = 0;
  int pol_idx = -1;

  if (find_ue_sst(UE, &ue_sst) == 0)
    pol_idx = find_policy_idx_by_sst(nr_mac, ue_sst);

  rc_slice_csv_write_alloc(frame,
                           slot,
                           "retransmission",
                           UE->rnti,
                           ue_sst,
                           ...
                           sched_ctrl->sched_pdsch.tb_size,
                           sched_ctrl->sched_pdsch.mcs,
                           sched_ctrl->num_total_bytes,
                           -1,
                           -1,
                           0,
  ...
  *n_rb_sched -= sched_ctrl->sched_pdsch.rbSize;
  ...
  const uint16_t retx_slbitmap = SL_to_bitmap(retInfo->tda_info.startSymbolIndex,
                                              retInfo->tda_info.nrOfSymbols);
  for (int rb = rbStart; rb < rbStart + sched_ctrl->sched_pdsch.rbSize; rb++)
    rballoc_mask[rb] |= retx_slbitmap;
...
static int slice_usage_index(int pol_idx, int beam_idx, int num_beams)
{
  return pol_idx * num_beams + beam_idx;
}


static const char *rc_slice_csv_path(void)
{
  const char *env_path = getenv("RC_SLICE_CSV_PATH");
  if (env_path && env_path[0] != '\0')
    return env_path;

  return RC_SLICE_CSV_DEFAULT_PATH;
}
...
static void rc_slice_csv_write_alloc(frame_t frame,
                                     ...
                                     uint16_t constrained_max_rb)
{
#if RC_SLICE_CSV_ENABLE
  static int csv_initialized = 0;

  const char *path = rc_slice_csv_path();
  ...
  if (!csv_initialized && pos == 0) {
    fprintf(fp,
            "frame,slot,event,rnti,sst,policy_idx,beam_idx,"
            "rb_start,rb_size,tbs_bytes,mcs,buffer_bytes,"
            "slice_used,slice_max,constrained_min_rb,constrained_max_rb\n");
    csv_initialized = 1;
  } else if (!csv_initialized) {
    csv_initialized = 1;
  }

  fprintf(fp,
          "%d,%d,%s,%04x,%u,%d,%d,%d,%d,%u,%d,%d,%d,%d,%u,%u\n",
          frame,
          slot,
          event ? event : "unknown",
          rnti,
...
static int ratio_to_rb(int total_rb, int ratio)
{
  if (ratio < 0)
    return 0;
  if (ratio > 100)
    ratio = 100;
  return (total_rb * ratio) / 100;
}
...
static int find_ue_sst(const NR_UE_info_t *UE, uint8_t *out_sst)
{
  ...
  for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); i++) {
    ...
    /* DRB/DTCH 위주로 slice 정보 확인 */
    if (lc->lcid != DL_SCH_LCID_DTCH)
      continue;

    *out_sst = lc->nssai.sst;
    return 0;
  }
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  int initial_rb_per_beam[num_beams];
  for (int i = 0; i < num_beams; i++)
    initial_rb_per_beam[i] = n_rb_sched[i];

  int slice_min_rb[MAX_RC_SLICE_POLICY][num_beams];
  int slice_max_rb[MAX_RC_SLICE_POLICY][num_beams];
  memset(slice_min_rb, 0, sizeof(slice_min_rb));
  memset(slice_max_rb, 0, sizeof(slice_max_rb));

  static int usage_frame = -1;
  static int usage_slot = -1;
  static int usage_num_beams = -1;
  static int *persist_slice_used_rb = NULL;

  const int usage_len = MAX_RC_SLICE_POLICY * num_beams;

  if (persist_slice_used_rb == NULL || usage_num_beams != num_beams) {
    free(persist_slice_used_rb);
    persist_slice_used_rb = calloc(usage_len, sizeof(int));
    AssertFatal(persist_slice_used_rb != NULL, "persist_slice_used_rb allocation failed\n");
    usage_num_beams = num_beams;
    usage_frame = -1;
    usage_slot = -1;
  }

  if (usage_frame != frame || usage_slot != slot) {
    memset(persist_slice_used_rb, 0, usage_len * sizeof(int));
    usage_frame = frame;
    usage_slot = slot;
  }
  ...
  pthread_mutex_lock(&mac->rc_slice_policy.lock);
  {
    ...
    static uint8_t last_n_policy = 0;
    static uint8_t last_valid[MAX_RC_SLICE_POLICY] = {0};
    static uint8_t last_sst[MAX_RC_SLICE_POLICY] = {0};
    static uint8_t last_min[MAX_RC_SLICE_POLICY] = {0};
    static uint8_t last_max[MAX_RC_SLICE_POLICY] = {0};

    int changed = !printed_once || (last_n_policy != mac->rc_slice_policy.n_policy);

    for (int p = 0; p < mac->rc_slice_policy.n_policy && p < MAX_RC_SLICE_POLICY; p++) {
      nr_rc_slice_policy_t *pol = &mac->rc_slice_policy.policy[p];

      if (pol->valid) {
        for (int b = 0; b < num_beams; b++) {
          slice_min_rb[p][b] = ratio_to_rb(initial_rb_per_beam[b], pol->min_ratio);
          slice_max_rb[p][b] = ratio_to_rb(initial_rb_per_beam[b], pol->max_ratio);
        }
      }

      if (!printed_once ||
          last_valid[p] != pol->valid ||
          last_sst[p] != pol->sst ||
          last_min[p] != pol->min_ratio ||
          last_max[p] != pol->max_ratio) {
        changed = 1;
      }
    }

    if (changed) {
      printf("[DL-SCHED] updated slice policies:");
      for (int p = 0; p < mac->rc_slice_policy.n_policy && p < MAX_RC_SLICE_POLICY; p++) {
        nr_rc_slice_policy_t *pol = &mac->rc_slice_policy.policy[p];
        if (!pol->valid)
          continue;

        printf(" [pol=%d sst=%u min=%u%% max=%u%%",
               p, pol->sst, pol->min_ratio, pol->max_ratio);

        for (int b = 0; b < num_beams; b++)
          printf(" beam%d(%d~%d)", b, slice_min_rb[p][b], slice_max_rb[p][b]);
      ...
      printed_once = 1;
      last_n_policy = mac->rc_slice_policy.n_policy;

      for (int p = 0; p < MAX_RC_SLICE_POLICY; p++) {
        if (p < mac->rc_slice_policy.n_policy) {
          nr_rc_slice_policy_t *pol = &mac->rc_slice_policy.policy[p];
          last_valid[p] = pol->valid;
          last_sst[p] = pol->sst;
          last_min[p] = pol->min_ratio;
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      if (sch_ret)
        sch_ret = allocate_dl_retransmission(module_id,
                                             frame,
                                             slot,
                                             &n_rb_sched[beam.idx],
                                             UE,
  ...
  while (iterator->UE != NULL) {
    ...
    if (find_ue_sst(iterator->UE, &ue_sst) == 0)
      pol_idx = find_policy_idx_by_sst(mac, ue_sst);

    sched_pdsch->time_domain_allocation = get_dl_tda(mac, slot);
    AssertFatal(sched_pdsch->time_domain_allocation >= 0,
    ...
    const uint16_t slbitmap = SL_to_bitmap(tda_info->startSymbolIndex,
    ...
    if (!get_FeedbackDisabled(iterator->UE->sc_info.downlinkHARQ_FeedbackDisabled_r17,
                              sched_pdsch->dl_harq_pid)) {
      int r_pucch = nr_get_pucch_resource(sched_ctrl->coreset,
                                          ul_bwp->pucch_Config,
                                          CCEIndex);

      alloc = nr_acknack_scheduling(mac,
                                    iterator->UE,
                                    frame,
                                    slot,
                                    iterator->UE->UE_beam_index,
    ...
    uint16_t constrained_min_rb = min_rbSize;
    uint16_t constrained_max_rb = max_rbSize;
    ...
    if (pol_idx >= 0) {
      used_idx = slice_usage_index(pol_idx, beam.idx, num_beams);
      used_before = persist_slice_used_rb[used_idx];

      slice_min = slice_min_rb[pol_idx][beam.idx];
      slice_max = slice_max_rb[pol_idx][beam.idx];

      int remaining_slice_max = slice_max - used_before;
      int remaining_slice_min = slice_min - used_before;

      if (remaining_slice_max <= 0) {
        printf("[DL-SCHED][%d.%d] UE %04x skipped: sst=%u pol=%d slice budget exhausted used=%d/%d\n",
               frame, slot, rnti, ue_sst, pol_idx, used_before, slice_max);
      ...
      if (remaining_slice_max < constrained_max_rb)
        constrained_max_rb = remaining_slice_max;

      if (remaining_slice_min > 0 && remaining_slice_min > constrained_min_rb)
        constrained_min_rb = remaining_slice_min;

      if (constrained_min_rb > constrained_max_rb)
        constrained_min_rb = constrained_max_rb;

      if (constrained_max_rb < min_rbSize) {
        printf("[DL-SCHED][%d.%d] UE %04x skipped: sst=%u pol=%d constrained_max_rb=%u < min_rbSize=%d used=%d/%d\n",
               frame,
               slot,
               rnti,
               ue_sst,
    ...
    bool ok = nr_find_nb_rb(sched_pdsch->Qm,
                            sched_pdsch->R,
                            1,
                            sched_pdsch->nrOfLayers,
                            tda_info->nrOfSymbols,
                            sched_pdsch->dmrs_parms.N_PRB_DMRS *
                            sched_pdsch->dmrs_parms.N_DMRS_SLOT,
    ...
    /*
     * Step 1/2 policy behavior:
     * - pol_idx < 0: no xApp slice policy is installed for this UE yet.
     *   Keep the original PF-like behavior. Do not reject the allocation
     *   only because ok == false. nr_find_nb_rb() can still return a valid
     *   partial allocation through rbSize/TBS.
     * - pol_idx >= 0: slice policy exists. The min/max constraints above
     *   have already been applied through constrained_min_rb/max_rb.
     */
    if (rbSize == 0 || TBS <= oh) {
      if (pol_idx >= 0 || RC_SLICE_SHOULD_LOG(frame, slot)) {
        printf("[DL-SCHED][%d.%d] UE %04x skipped: invalid allocation, "
               "sst=%u pol=%d min=%u max=%u bytes=%d ok=%d TBS=%u rbSize=%u oh=%d\n",
               frame,
               slot,
               rnti,
               ue_sst,
    ...
    if (!ok && RC_SLICE_SHOULD_LOG(frame, slot)) {
      printf("[DL-SCHED][%d.%d] UE %04x partial allocation accepted: "
             "sst=%u pol=%d bytes=%d TBS=%u rbSize=%u oh=%d\n",
             frame,
             slot,
             rnti,
             ue_sst,
    ...
    if (pol_idx >= 0) {
      persist_slice_used_rb[used_idx] += rbSize;

      if (RC_SLICE_SHOULD_LOG(frame, slot)) {
        printf("[DL-SCHED][%d.%d] UE %04x policy applied: sst=%u pol=%d rbSize=%u slice_used=%d->%d/%d slice_min=%d constrained=%u~%u TBS=%u\n",
               frame,
               slot,
               rnti,
               ue_sst,
               ...
               rbSize,
               used_before,
               persist_slice_used_rb[used_idx],
               slice_max,
               slice_min,
               constrained_min_rb,
               constrained_max_rb,
...
//--------------------------------------------------------------------------------------------------------------
    int csv_slice_used = -1;
    int csv_slice_max = -1;

    if (pol_idx >= 0 && used_idx >= 0) {
      csv_slice_used = persist_slice_used_rb[used_idx];
      csv_slice_max = slice_max;
    }
    ...
    for (int rb = 0; rb < sched_pdsch->rbSize; rb++)
      rballoc_mask[bwp_start + sched_pdsch->rbStart + rb] |= slbitmap;
