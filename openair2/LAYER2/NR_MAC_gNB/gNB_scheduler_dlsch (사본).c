                  ...
                  int n_rb_sched[num_beams])
{
  ...
  /*
   * 같은 frame.slot 동안 유지되는 slice usage
   * pf_dl() 호출이 여러 번 있어도 같은 slot이면 usage 유지
   */
  ...
  /* policy store에서 slot 기준 slice budget 계산 + 정책 변경 시에만 출력 */
  ...
  while (iterator->UE != NULL) {
    ...
    uint16_t orig_rbSize = rbSize;

    if (pol_idx >= 0) {
      int used_idx = slice_usage_index(pol_idx, beam.idx, num_beams);
      int used_before = persist_slice_used_rb[used_idx];
      int remaining_slice_max = slice_max_rb[pol_idx][beam.idx] - used_before;
      int remaining_slice_min = slice_min_rb[pol_idx][beam.idx] - used_before;
      ...
      if (rbSize > remaining_slice_max)
        rbSize = remaining_slice_max;

      if (remaining_slice_min > 0) {
        int target_rb = remaining_slice_min;
        if (target_rb > remaining_slice_max)
          target_rb = remaining_slice_max;
        if (target_rb > max_rbSize)
          target_rb = max_rbSize;

        if (rbSize < target_rb)
          rbSize = target_rb;
      }

      if (rbSize > max_rbSize)
        rbSize = max_rbSize;

      printf("[DL-SCHED][%d.%d] UE %04x policy applied: sst=%u pol=%d orig_rbSize=%u final_rbSize=%u slice_used=%d->%d/%d slice_min=%d\n",
             frame,
             slot,
             rnti,
             ue_sst,
             ...
             used_before,
             used_before + rbSize,
             slice_max_rb[pol_idx][beam.idx],
             slice_min_rb[pol_idx][beam.idx]);

      persist_slice_used_rb[used_idx] += rbSize;

      printf("[DL-SCHED][%d.%d] UE %04x slice usage updated: sst=%u pol=%d slice_used=%d/%d rbSize=%u\n",
             frame,
             slot,
             rnti,
             ue_sst,
