...
//------------------------------------------------------------------------------------------------
  ...
  ue_prb_log_t ue_prb_log[MAX_MOBILES_PER_GNB];
  int ue_prb_log_len = 0;
//---------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
    if (RC_SLICE_SHOULD_LOG(frame, slot)) {
      printf("[DL-SCHED-SUM][%d.%d]", frame, slot);

      for (int p = 0; p < mac->rc_slice_policy.n_policy && p < MAX_RC_SLICE_POLICY; p++) {
        int used_idx = slice_usage_index(p, 0, num_beams);

        printf(" slice%d used=%d/%d:",
               p,
               persist_slice_used_rb[used_idx],
               slice_max_rb[p][0]);

        for (int i = 0; i < ue_prb_log_len; i++) {
          if (ue_prb_log[i].pol_idx == p) {
            printf(" UE%04x=%dPRB",
                   ue_prb_log[i].rnti,
                   ue_prb_log[i].rb_sum);
          }
...
//------------------------------------------------------------------------------------------------------------
    ...
    if (rbSize == 0 || TBS == 0) {
      printf("[DL-SCHED][%d.%d] UE %04x skipped: nr_find_nb_rb produced invalid result, sst=%u pol=%d min=%u max=%u bytes=%d ok=%d TBS=%u rbSize=%u\n",
             frame,
             slot,
             rnti,
             ue_sst,
 ...
 //--------------------------------------------------------------------------------------- ---------------------  -
    int found = 0;
    for (int i = 0; i < ue_prb_log_len; i++) {
      if (ue_prb_log[i].rnti == rnti) {
        ue_prb_log[i].rb_sum += rbSize;
        found = 1;
        break;
      }
    }

    if (!found && ue_prb_log_len < MAX_MOBILES_PER_GNB) {
      ue_prb_log[ue_prb_log_len].rnti = rnti;
      ue_prb_log[ue_prb_log_len].sst = ue_sst;
      ue_prb_log[ue_prb_log_len].pol_idx = pol_idx;
      ue_prb_log[ue_prb_log_len].rb_sum = rbSize;
      ue_prb_log_len++;
    }
//---------------------------------------------------------------------------------------------------------------    
